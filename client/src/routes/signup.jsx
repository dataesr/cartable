...
export default function Passwordless({ firstConnexion }) {
  const title = firstConnexion ? 'Première connexion' : 'Mot de passe oublié';
  usePageTitle(title);
  const navigate = useNavigate();
  const { requestPasswordChangeEmail, changePassword } = useAuth();
  ...
  const handleChangePassword = async (e) => {
    ...
    if (!firstName || !lastName) return setChangePasswordError('Le nom et le prénom sont obligatoires');
    const response = await changePassword({ email, password, otp, firstName, lastName });
                      ...
                      <form onSubmit={handleChangePassword}>
                        ...
                        <Row spacing="my-2w">
                          <Col>
                            <ButtonGroup>
                              <Button submit>
                                {firstConnexion ? 'Activer mon compte' : 'Valider'}
                              </Button>
                            </ButtonGroup>
                          </Col>
                        </Row>
              ...
              {(step === 4) && (
                <>
                  <Row justifyContent="center">
                    <Col>
                      <Stepper
                        currentStep={step}
                        steps={4}
                        currentTitle={(firstConnexion) ? 'Mot de passe enregistré' : 'Mot de passe réinitialisé'}
                        nextStepTitle=""
                      />
                    </Col>
                  </Row>
                  <Row spacing="py-6w">
                    ...
                    />
                    <Text bold size="lg">
                      {(firstConnexion) ? 'Mot de passe enregistré !' : 'Mot de passe réinitialisé !'}
                    </Text>
                    <Highlight colorFamily="green-emeraude">
                      Vos informations ont été enregistrés avec succès.
                      {' '}
                      <br />
...
Passwordless.propTypes = {
  firstConnexion: PropTypes.bool.isRequired,
};
